...
func run(ctx context.Context) error {
    ...
    //Helper that runs a command in the current container state, prints stdout/stderr,
        run := func(name string, cmd []string, artifact string) (*dagger.Container, error) {
            ...
            if execErr != nil {
                if errout != "" { fmt.Printf("[%s] STDERR:\n%s\n", name, errout) }
                if out != "" { fmt.Printf("[%s] STDOUT:\n%s\n", name, out) }
                if artifact != "" {
                    ...
                    if clog != "" { fmt.Printf("[%s] ARTIFACT %s:\n%s\n", name, artifact, clog) }
                }
    ...
    if _, err := run("python-version", []string{"python", "--version"}, ""); err != nil { return err }
    ...
    if _, err := run("data", dataCmd, "artifacts/data_py_error.log"); err != nil { return err }
    ...
    if _, err := run("prepare-data", prep, ""); err != nil { return err }
    ...
    if _, err := run("train", trainCmd, "artifacts/train_run.log"); err != nil { return err }

    //Export the container `artifacts/` directory to the host repo `../artifacts`.
    artDir := current.Directory("artifacts")
    if _, err := artDir.Export(ctx, "../artifacts"); err != nil { return fmt.Errorf("export failed: %w", err) }
    ...
    if _, err := run("evaluate", evalCmd, "artifacts/evaluate_py_error.log"); err != nil { return err }
    ...
    if _, err := run("deploy", deployCmd, "artifacts/deploy_py_error.log"); err != nil { return err }
